...
export default (props) => {
  const state = useState({
    showAddEditProject: false
  })
  const toggleAddEditProject = ()=>{
    state.showAddEditProject.set(!state.showAddEditProject.value);
  }
  return (
    <>
    {state.showAddEditProject.value?
    <AddEditProject toggle={toggleAddEditProject} show={state.showAddEditProject.value}/>:
    null}
      <div className="d-flex justify-content-between flex-wrap flex-md-nowrap align-items-center py-4">
        <Dropdown>
          ...
          <Dropdown.Menu className="dashboard-dropdown dropdown-menu-left mt-2">
            <Dropdown.Item as="button" onClick={()=>toggleAddEditProject()}>Project</Dropdown.Item>
            <Dropdown.Item>
      ...
      <Row>
        <Col xs={12} xl={8}>
          <button className="btn btn-primary" onClick={()=>{
            let route = Routes.ProjectDetails.clean+"1";
